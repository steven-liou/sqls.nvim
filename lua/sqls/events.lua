...
function M.add_subscriber(event_name, subscriber)
    vim.notify_once(
        'sqls.nvim: the "add_subscriber()" function is deprecated, use sqls.nvim user events instead (:h sqls-nvim-events)',
        vim.log.levels.WARN
    )
    vim.validate({
...
function M.remove_subscriber(event_name, subscriber)
    vim.notify_once(
        'sqls.nvim: the "remove_subscriber()" function is deprecated, use sqls.nvim user events instead (:h sqls-nvim-events)',
        vim.log.levels.WARN
    )
    vim.validate({
        event_name = { event_name, "string" },
