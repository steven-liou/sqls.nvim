...
M.on_attach = function(client, bufnr)
    ...
    api.nvim_buf_create_user_command(bufnr, "SqlsDescribeTable", function()
        require("sqls.commands").table_helper(client_id, "describe_table")
    end, { nargs = "?" })
    api.nvim_buf_create_user_command(bufnr, "SqlsForeignKeys", function()
        require("sqls.commands").table_helper(client_id, "foreign_keys")
    end, { nargs = "?" })
    api.nvim_buf_create_user_command(bufnr, "SqlsPrimaryKeys", function()
        require("sqls.commands").table_helper(client_id, "primary_keys")
    end, { nargs = "?" })
    api.nvim_buf_create_user_command(bufnr, "SqlsUnique", function()
        require("sqls.commands").table_helper(client_id, "unique")
    end, { nargs = "?" })
    api.nvim_buf_create_user_command(bufnr, "SqlsReferences", function()
        require("sqls.commands").table_helper(client_id, "references")
    end, { nargs = "?" })
    api.nvim_buf_create_user_command(bufnr, "SqlsIndices", function()
        require("sqls.commands").table_helper(client_id, "indices")
    end, { nargs = "?" })
    api.nvim_buf_create_user_command(bufnr, "SqlsList", function()
        require("sqls.commands").table_helper(client_id, "list")
    end, { nargs = "?" })
...
M.commands = {
    ...
    describeTable = function(_, client)
        require("sqls.commands").describe_table(client.client_id)
    end,
